...
//    @Test
    public void quotesReturnedWhenParamsAreValid() throws Exception {
        ...
        Collection<IQuote> quotes = quotesService.getQuotes(new HashSet<>(Arrays.asList(
    ...
    public void buildTickerQueryStringCorrectlyAddsExchangeSuffix() {
        ...
        tickers.add(new Ticker(ObjectId.get().toHexString(), "VWO", Currency.USD.name(), Exchange.NYSEARCA.name(), ""));
        tickers.add(new Ticker(ObjectId.get().toHexString(), "CPJ1", Currency.GBp.name(), Exchange.LSE.name(), ""));
